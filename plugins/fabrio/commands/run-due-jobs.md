...
2. **A user-scope tool allow-list.** In `-p` mode Claude Code cannot prompt, so any tool not allow-listed fails. Put the list in **`~/.claude/settings.json`** (user scope, so it applies wherever a child runs): `mcp__fabrio` (all its tools) plus `Bash(claude:*)`. `http_api` connectors are read through `mcp__fabrio`'s `connector_fetch` tool — no per-connector MCP server to register — but a job that reads a `remote_mcp` / `local_mcp` resource still needs that server; see **Fabrio → Settings → Tools** for the authoritative set. A tool a job needs but the machine does not permit fails at the moment it is used, not at startup, so it surfaces as a run that got part-way and stopped. A **keyed** `http_api` connector still needs its API key in `~/.fabrio/credentials.json` — `connector_fetch` receives it as a transient argument the run reads locally, never a stored Fabrio value.
