...
> **Never prompt interactively.** This skill is dispatched headlessly by `/fabrio:ops-heartbeat`, where a prompt hangs the whole cycle. A question recorded with `create_task_question` is the durable equivalent and works identically attended or headless.
...
```bash
claude -p "/fabrio:feature-request {task_number}" --permission-mode acceptEdits
```
...
> Running attended and already inside the repo? Invoking `/fabrio:feature-request {n}` directly in-session is equivalent — the dispatch form exists so headless children get a clean context.
