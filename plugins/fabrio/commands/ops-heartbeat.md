...
2. **A user-scope tool allow-list.** In `-p` mode Claude Code cannot prompt, so any tool not allow-listed fails. Put the list in **`~/.claude/settings.json`** (user scope — children run in each site's repo dir, where a project `.claude/settings.json` wouldn't apply): `mcp__fabrio` (all its tools) plus `Bash(git:*)`, `Bash(gh:*)`, `Bash(npm run:*)`, `Bash(npx:*)`, `Bash(claude:*)`.
   ...
   **This list must cover whatever the workspace's tool catalog grants** — see **Fabrio → Settings → Tools** for the authoritative set, including any tools this workspace added (`Bash(dotnet:*)`, an internal MCP server). A tool an agent is assigned but the machine does not permit fails at the moment it is used, not at startup, so it surfaces as a task that got most of the way and then stopped. Conversely, a machine can *refuse* a granted tool on purpose: set `deny_tools` in `~/.fabrio/config.json` (or `FABRIO_DENY_TOOLS`) and `fabrio-runner` strips it before spawning, logging what it cut. That veto belongs to whoever owns the machine and cannot be overridden from Fabrio.
