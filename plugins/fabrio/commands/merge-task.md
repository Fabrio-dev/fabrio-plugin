...
## Step 8.6 — Report What Isn't This Ticket
...
|---|---|
| you lacked knowledge you should have had, and would repeat the mistake | `record_learning` / `reinforce_learning` — Step 8.5 above |
...
| **this ticket is under-specified and you cannot proceed** | the blocking `create_task_question` / `create_decision` path in the executor skills — unchanged |
...
At merge time the highest-value finding is usually **follow-up work the review surfaced but the PR deliberately left out**. That is a suggested ticket, not a learning — a learning changes how the next run works, a ticket is the work itself.
