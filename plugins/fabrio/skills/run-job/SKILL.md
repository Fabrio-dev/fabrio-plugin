...
2. **Preflight by `access_method` — check, never prompt:**
   ...
   - `http_api` **with an `operations` list** (from `get_resource` / `list_site_resources`) → **queryable.** Preflight by calling the operation whose `health_check` is true:
     `connector_fetch { resource_id, operation: "<that key>" }`. For a **keyed** operation (its `auth` is set), first read the credential named in `auth.credential` from `~/.fabrio/credentials.json` at `auth.credential_path` and pass it as `credentials: { "<KEY>": "<value>" }` — it is used for that one request and never stored. `ok: false` with `needs_credential` or a 401/403 → `unauthenticated`; a 5xx / timeout / transport error → `unreachable`. A health-check operation records the connection status itself, so you don't also call `record_resource_check` for it.
   - `http_api` with **no `operations`** but whose **notes** spell out the exact calls → follow the notes (treat a missing `credential_keys` name in `~/.fabrio/credentials.json` as `unauthenticated`; read only key **names**, never echo a value).
   - `http_api` with **neither** operations nor notes, and `dashboard_link` → **reference-only.** Nothing to preflight and a passing credential check would be a false green. If a Fetch step depends on one, fail with `"Source unavailable: <name> is reference-only"` and say to re-run `$fabrio:plan-job {item_number}`.

3. **On success**, call `record_resource_check { resource_id, status: "ok" }` and continue (a `connector_fetch` health-check operation already did this — don't repeat it). This is the only way the Fabrio UI ever learns this machine can reach the resource — the browser can't see your local MCP servers or credentials file.

4. **On failure, stop cleanly. Never prompt** — `$fabrio:run-due-jobs` dispatches this skill through Codex agent delegation and a prompt would hang the whole cycle. Make the calls below, then report and stop:
...
When the step reads an `http_api` connector, call **`connector_fetch { resource_id, operation, params, site_id }`** — the step's `instructions` name the operation and params, and `get_resource` / `list_site_resources` carries the full `operations` list (each with its `url`, `required_params`, `auth`, `paginate`, `returns`). Pass `site_id` so per-site config (series, sports, …) is applied. For a keyed operation, read the credential locally per its `auth.credential_path` and pass `credentials: { "<KEY>": "<value>" }` — transient, never store or echo it. Cursor pages are followed for you up to `max_pages` (default 5; raise it, ≤ 20, if `truncated` comes back). A mid-run `needs_credential` / 401 / 5xx is a **source failure** — handle it as in Step 1.5 §4.
...
- `description` — **everything the executor needs**, in **≤ 12000 characters.** Carry the evidence and the decisions this run made: IDs, counts, affected areas, links, repro steps, the chosen topic and why, the brief. The task **cannot read the job's steps** — whatever your planning steps worked out is lost unless you write it here, and the executor will re-derive it (differently) if you don't. Triage jobs legitimately spend that budget on verbatim stack traces and repro steps; if even 12000 chars isn't enough, link to the source (the PostHog issue, the log) rather than pasting more raw data as prose.
...
Never prompt — `$fabrio:run-due-jobs` dispatches this skill through Codex agent delegation. Every exit is a durable artifact.
