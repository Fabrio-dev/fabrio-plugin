...
Only propose a **new** generator when the objective genuinely needs recurring, source-driven work that files a ticket per issue (e.g. "keep beta bugs low"); set `kind: "generator"`, put a clear "how we gather work" account in the item's `description`, and note that the human runs `/fabrio:plan-job` on it after accepting to author its steps.

**An item is an initiative, not a stage.** A recurring pipeline — *fetch → analyse → classify → file* — is **one** generator item whose stages are its steps, never a chain of sibling items. Never give a generator a `depends_on`; a generator runs its whole pipeline in a single run, so a stage of it can't be a prerequisite for it. If a `depends_on` chain in the current plan runs more than two links deep, or a recurring item depends on `one_time` items that restate its own procedure, that is the bug — propose the collapsed version.
