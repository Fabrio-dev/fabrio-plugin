...
All data access is through the **`fabrio` MCP server** (`mcp__fabrio__*` tools) — **no Supabase CLI and no `SUPABASE_DB_URL` anymore.** The server scopes to the connected account; `scripts/use-account.ps1 <name>` selects which one. If the tools aren't available, stop and tell the user to connect an account.
